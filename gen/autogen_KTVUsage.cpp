...
void KTVMain::Usage(void)
{
	...
	printf("    could write --sdelim $\'\\t \' to delimit with tab and space. It is best to\n");
	printf("    use the -t option to avoid shell issues and promote portability.\n");
	printf("\n");
	printf("    -d --delim string\n");
	printf("    -s --sdelim string\n");
	printf("         The file will be parsed using the specified characters as\n");
	printf("         delimiters. With --delim adjacent delimiters are merged, while with\n");
	printf("         --sdelim they are not. DEFAULT= --delim with tab and space.\n");
	printf("\n");
	printf("    -t   Add a tab to the delimiter string. Some shells make it challenging\n");
	printf("         to pass a tab to the program. If that is the case, this may be used\n");
	printf("         instead. If -d or -s is used as well, tab is added as a delimiter\n");
	printf("         to the existing choices. If not, then -d is implied. To parse only\n");
	printf("         with single tabs, use -s \'\' -t or something similar. The -t option\n");
	printf("         must follow any -d or -s options.\n");
	printf("\n");
	printf("    -c --csv\n");
	printf("         The file will be parsed according to the csv specification. NOTE:\n");
	printf("         At present, this is a simplified parser. It allows quoted or\n");
	printf("         non-quoted fields and can accommodate embedded ,\'s inside of quoted\n");
	printf("         fields -- but nothing more complex than this.\n");
	printf("\n");
	...
	printf("\n");
	printf("    -e --eol char\n");
	printf("         Use the specified character as the end of line marker in the file.\n");
	printf("         If unset, then any of \\n, \\r, or \\r\\n will match. Note that null\n");
	printf("         always matches and that adjacent eols (except \\r\\n) yield separate\n");
	printf("         lines and are not merged.\n");
	printf("\n");
	printf("    -M --maxcols n\n");
	...
	printf("         Note that this is automatic with --csv.\n");
	printf("\n");
	printf("    -k --skip n\n");
	...
	printf("         the first row (or first non-skipped row). Not for use with --fixed.\n");
	printf("         Note that this may lead to visually unappealing results if the\n");
	printf("         first row is a header with headings that are smaller than the data\n");
	printf("         entries below them. This may be corrected using the key commands\n");
	printf("         \"+\", \"-\",\"(\",\")\", and \"W\" as appropriate.\n");
	printf("\n");
	printf("    -w --defwidth n\n");
	printf("         Set the column width for all columns to n. DEFAULT=12.\n");
	printf("\n");
	printf("    -G --grid [c]\n");
	printf("         Toggle display separators between columns. The default is ON with\n");
	printf("         \'|\' as the separator. If \'c\' is specified, use that character as\n");
	printf("         the separator and keep the grid on. Otherwise, turn it off. Note\n");
	printf("         that the grid may be turned on and off dynamically as well (see key\n");
	printf("         command \"x\").\n");
	printf("\n");
	printf("    -S --seps [c]\n");
	printf("         Toggle display separators between the frozen rows/cols and the\n");
	printf("         active portion of the screen. The default is ON with \'#\' as the\n");
	printf("         separator. If \'c\' is specified, use that character as the separator\n");
	printf("         and keep it on. Otherwise, turn it off. The separators may be\n");
	printf("         toggled on and off dynamically as well (see key command \"z\").\n");
	printf("\n");
	...
	printf("         dynamically using the \"^\",\"$\",\"[\", and \"]\" key commands.\n");
	printf("         DEFAULT=right.\n");
	printf("\n");
	...
	printf("    command should provide full visibility where necessary. Regardless, the\n");
	printf("    matching cell row and col are displayed in the message bar. Also note\n");
	printf("    that regexps are compared per cell. That is, the cell itself is viewed\n");
	printf("    as a \'line\' rather than the row as a whole. Typically, the active window\n");
	printf("    aligns to the next search result in the upper left corner. However, it\n");
	printf("    may not do so (or appear to do so) if the result appears in the frozen\n");
	printf("    part of the screen or too few rows or columns remain to fill the screen.\n");
	...
	printf("    j,ret Move down one line.\n");
	...
	printf("    treated like an ordinary string to match. Note that the / used to\n");
	printf("    inaugurate the command is independent of the / that starts a regexp; in\n");
	printf("    other words, / must be hit twice to start a regexp.\n");
	printf("    S or ? [foo] Column-wise search. Places top-left corner at search result\n");
	printf("    and displays the result on the message line. The search is down and\n");
	printf("    forward from the upper left active cell (or the previous result). If foo\n");
	printf("    is of the form /bar/ then it is interpreted as a regexp. Otherwise it is\n");
	...
	printf("    contents.\n");
	printf("    H [n] Toggle hidden status of column n. Hidden columns are not shown or\n");
	printf("    searched and may or may not be frozen.\n");
	...
	printf("    drop below 1.\n");
	printf("    ) Expand width of all columns by 1 (same as \"+ .\")\n");
	printf("    ( Shrink width of all columns by 1 (same as \"- .\")\n");
	printf("    W [n:w] Specify width of column n as w.\n");
	printf("    W [.:w] Specify width of all columns as w.\n");
	printf("    W [w] Specify width of all columns as w (same as \"W .:w\")\n");
	...
	printf("    [ Justify all columns to left (same as \"^ .\")\n");
	printf("    ] Justify all columns to right (same as \"$ .\")\n");
	...
	printf("\n");
	printf("    Note that the G and S commands necessarily involve a seek to the end of\n");
	printf("    the file and will be expensive for large files. Also, the commands s, n,\n");
	printf("    p, and L may prove expensive depending on the details of the file and\n");
	printf("    request.\n");
	printf("\n");
	printf("Key Summary\n");
	printf("     Action   <--Cols-->   <---------Rows---------->\n");
	printf("     -----------------------------------------------\n");
	printf("     Move    a < l r > e   g b|u y|k j|ret f|d|spc G\n");
	printf("     (Un)Freeze  C [n|.]            R [n|.]         \n");
	printf("     (Un)Hide    H [n|.]                            \n");
	printf("     Search  S|? [str|/str/]      s|/ [str|/str/]   \n");
	printf("     -----------------------------------------------\n");
	printf("     Col width:  +/-[n|.]  ( )   W[n:w] W[.:w] W[w]\n");
	...
	printf("\n");
	printf("    Parse tab-delimited: tless -f foo.txt -s \"\" -t -a\n");
	printf("\n");
	printf("    Parse white-space separated columns: tless -f foo.txt -d \" \" -t -a\n");
	printf("\n");
	printf("    Parse left-justified fixed-width columns: tless -f foo.txt -j left\n");
	printf("    -x:10:10:15:8:15:6\n");
	printf("\n");
	printf("    Parse \':\' separated columns that are all the same width: tless -f\n");
	printf("    foo.txt -d \":\" -w 15\n");
	printf("\n");
	printf("    A simple setup tless -f foo.txt -X \"C:0 R:0 L:24:10\"\n");
	printf("\n");
	printf("EXIT STATUS\n");
